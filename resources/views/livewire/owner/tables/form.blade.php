<form wire:submit="save" class="space-y-5">
    ...
    <div>
        <label for="table-name" class="mb-2 block text-xs font-black uppercase tracking-[0.2em] text-slate-500">{{ __('owner.tables.name') }}</label>
        <input wire:model.blur="name" id="table-name" type="text" placeholder="{{ __('owner.tables.name_placeholder') }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none shadow-sm transition-all duration-200">
        @error('name')
    ...
    <div>
        <p class="mb-2 text-xs font-black uppercase tracking-[0.2em] text-slate-500">{{ __('owner.common.status') }}</p>
    ...
    <button type="submit" wire:loading.attr="disabled" wire:target="save"
        ...
        {{ $tableId ? __('owner.tables.save_changes') : __('owner.tables.create') }}
    </button>
</form>
