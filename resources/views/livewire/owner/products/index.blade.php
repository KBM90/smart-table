    ...
    <section class="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl shadow-slate-950/40">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <p class="mt-2 max-w-2xl text-sm text-slate-300">Manage your browse-only menu catalog with active
                    states, sort order, uploads, and built-in image picks.</p>
            </div>

            <button wire:click="createProduct" type="button"
                class="rounded-xl bg-amber-500 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-amber-400">
                Create product
            </button>
        </div>
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2">
            <label class="block">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cappuccino"
                    class="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white placeholder:text-slate-500 focus:border-amber-400 focus:outline-none focus:ring-0">
            </label>
                    ...
                    @forelse ($products as $product)
                        <tr class="align-top">
                            ...
                            <td class="px-6 py-4 text-sm text-slate-300">
                                @if ($product->category)
                                    <span
                                        class="inline-flex rounded-full bg-slate-800 px-3 py-1 text-xs font-semibold text-slate-200">
                            ...
                            <td class="px-6 py-4">
                                <button wire:click="toggleActive({{ $product->id }})" type="button"
                                    class="inline-flex rounded-full px-3 py-1 text-xs font-semibold {{ $product->is_active ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-700 text-slate-300' }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap justify-end gap-2">
                                    <button wire:click="editProduct({{ $product->id }})" type="button"
                                        class="rounded-lg border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-200 hover:border-amber-400 hover:text-amber-300">Edit</button>
                                    <button wire:click="deleteProduct({{ $product->id }})" type="button"
                                        class="rounded-lg border border-rose-500/40 px-3 py-2 text-xs font-semibold text-rose-200 hover:border-rose-400 hover:text-rose-100">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-sm text-slate-400">No products found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
                ...
                <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                    <div class="mb-4 flex items-center justify-between">
                        ...
                        <button wire:click="closeForm" type="button"
                            class="text-sm font-medium text-slate-400 hover:text-white">Close</button>
                    </div>
