            ...
            <table class="min-w-full text-left border-collapse">
                <thead>
                    <tr
                        ...
                        <th scope="col" class="px-6 py-5">{{ __('owner.common.table_session') }}</th>
                        <th scope="col" class="px-6 py-5">{{ __('owner.common.status') }}</th>
                        <th scope="col" class="px-6 py-5">{{ __('owner.common.wait_time') }}</th>
                        <th scope="col" class="px-6 py-5 text-right">{{ __('owner.common.actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse ($requests as $request)
                        <tr wire:key="request-{{ $request->id }}"
                            class="group/row transition-colors duration-200 hover:bg-slate-50/50">
                            <td class="px-6 py-5 align-middle">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        @if ($request->order)
                                            <div
                                                class="mt-2 max-w-xs rounded-lg border border-indigo-100 bg-indigo-50/60 px-2.5 py-2">
                                                <p
                                                ...
                                                <ul class="mt-1 space-y-0.5 text-[11px] text-slate-600">
                                                    @foreach ($request->order->items as $item)
                                                        <li class="flex justify-between gap-2">
                                                            <span class="truncate">{{ $item->quantity }}x
                                                ...
                                                @if ($request->order->note)
                                                    <p class="mt-1 text-[10px] italic text-slate-500">
                                                        &ldquo;{{ $request->order->note }}&rdquo;
                                                    </p>
                                                @endif
                                            </div>
                                        @endif
