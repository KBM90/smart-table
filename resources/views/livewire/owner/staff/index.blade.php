    ...
    <section class="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/60 p-6 shadow-2xl shadow-indigo-100/50 backdrop-blur-xl">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600 font-medium">Create tenant-scoped waiter accounts and revoke access with soft deletion when staff leave.</p>
            </div>

            <button wire:click="createWaiter" type="button" class="shrink-0 group inline-flex items-center gap-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-3 text-sm font-bold text-white shadow-xl shadow-indigo-600/30 hover:shadow-indigo-600/50 hover:-translate-y-0.5 active:scale-95 transition-all duration-300">
        ...
        <div class="mt-6 max-w-xl">
            <label class="block">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search waiter name or email..." class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none shadow-sm transition-all duration-200">
            </label>
        </div>
    </section>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-transparent">
                        @forelse ($waiters as $waiter)
                            <tr class="group/row transition-colors duration-200 hover:bg-slate-50/50">
                                ...
                                <td class="px-6 py-4 align-middle text-sm text-slate-600 font-medium">{{ $waiter->email }}</td>
                                <td class="px-6 py-4 align-middle">
                                ...
                                <td class="px-6 py-4 align-middle text-right">
                                    <div class="flex justify-end">
                                        <button wire:click="deleteWaiter({{ $waiter->id }})" type="button" class="rounded-xl border border-red-200 bg-red-50/50 px-3.5 py-2 text-xs font-bold text-red-600 hover:bg-red-50 hover:border-red-300 shadow-sm transition-all duration-200">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="mt-1 text-xs text-slate-400 max-w-xs leading-relaxed">Try adjusting your search query or creating a new waiter account.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
