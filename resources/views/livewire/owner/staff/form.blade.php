<form wire:submit="save" class="space-y-5">
    <div>
        <label for="waiter-name" class="mb-2 block text-xs font-black uppercase tracking-[0.2em] text-slate-500">{{ __('owner.staff.name') }}</label>
        <input wire:model.blur="name" id="waiter-name" type="text" placeholder="{{ __('owner.staff.name_placeholder') }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none shadow-sm transition-all duration-200">
        @error('name')
    ...
    <div>
        <label for="waiter-email" class="mb-2 block text-xs font-black uppercase tracking-[0.2em] text-slate-500">{{ __('owner.staff.email') }}</label>
        <input wire:model.blur="email" id="waiter-email" type="email" placeholder="{{ __('owner.staff.email_placeholder') }}" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none shadow-sm transition-all duration-200">
        @error('email')
            <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        <label for="waiter-password" class="mb-2 block text-xs font-black uppercase tracking-[0.2em] text-slate-500">{{ __('owner.staff.password') }}</label>
    ...
    <div>
        <label for="waiter-password-confirmation" class="mb-2 block text-xs font-black uppercase tracking-[0.2em] text-slate-500">{{ __('owner.staff.confirm_password') }}</label>
