...
@section('content')
    <div class="space-y-8">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h1 class="text-2xl font-black text-slate-900">{{ __('owner.settings.title') }}</h1>
                <p class="mt-1 text-sm text-slate-500">{{ __('owner.settings.intro') }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
            <form method="POST" action="{{ route('owner.settings.update') }}"
                ...
                <div class="border-b border-slate-100 pb-5">
                    <p class="text-xs font-black uppercase tracking-widest text-indigo-500">{{ __('owner.settings.business_info') }}</p>
                    <p class="mt-2 text-sm leading-6 text-slate-500">
                        {{ __('owner.settings.business_info_body') }}
                    </p>
                </div>

                <div class="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="name" class="text-sm font-bold text-slate-700">{{ __('owner.settings.business_name') }}</label>
                    ...
                    <div>
                        <label for="contact_email" class="text-sm font-bold text-slate-700">{{ __('owner.settings.business_email') }}</label>
                    ...
                    <div>
                        <label for="phone" class="text-sm font-bold text-slate-700">{{ __('owner.settings.business_phone') }}</label>
                    ...
                    <div class="sm:col-span-2">
                        <label for="address" class="text-sm font-bold text-slate-700">{{ __('owner.settings.street_address') }}</label>
                    ...
                    <div>
                        <label for="city" class="text-sm font-bold text-slate-700">{{ __('owner.settings.city') }}</label>
                    ...
                    <div>
                        <label for="country" class="text-sm font-bold text-slate-700">{{ __('owner.settings.country') }}</label>
                ...
                <div class="mt-6 flex justify-end border-t border-slate-100 pt-5">
                    <button type="submit"
                        ...
                        {{ __('owner.settings.save_changes') }}
                    </button>
                </div>
            </form>
            ...
            <aside class="space-y-4">
                <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-100">
                    <p class="text-xs font-black uppercase tracking-widest text-slate-400">{{ __('owner.settings.owner_account') }}</p>
                    ...
                    <a href="{{ route('profile.edit') }}"
                        ...
                        {{ __('owner.settings.edit_profile') }}
                    </a>
                </div>

                <div class="rounded-lg border border-indigo-100 bg-indigo-50 p-5">
                    <p class="text-sm font-black text-indigo-950">{{ __('owner.settings.data_scope') }}</p>
                    <p class="mt-2 text-sm leading-6 text-indigo-900/80">
                        {{ __('owner.settings.data_scope_body') }}
                    </p>
                </div>
            </aside>
        </div>
